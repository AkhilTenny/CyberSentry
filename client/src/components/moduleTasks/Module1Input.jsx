...
function Module1Input(props) {

  const {completeModule1Task} = useTaskApi();
  ...
  function taskCompleted(){
    completeModule1Task(props.taskIndex,4).then(res=>
      setStatus(true)
    )
  }
...
return (
  <div className=' ml-4 p-6 mt-5 mb-4 h-38 bg-purple-800 shadow-xl  w-2/3 items-center justify-between flex rounded-3xl'>   
  {
    ...
    :
    <div>
        <h1 className=" text-white text-2xl font-bold ">Completed Instalation?</h1>
        <button 
          onClick={taskCompleted}
          className="text-white bg-green-600 text-lg mt-4 px-2 rounded-xl py-1 font-bold ml-3">Completed</button>
